...
async function processUrl(url, index, total, cookies) {
  ...
  try {
    ...
    const msg = err.response ? `HTTP ${err.response.status}: ${err.message}` : err.message;
  ...
  try {
    ...
    if (!processed.error) {
      const { dailyCsvPath, sectorCsvPath, summaryPath } = writeBacktestOutput(processed, slug);
      const s = processed.summary;
      printStep('✔', `Date range: ${s.dateRange.from} → ${s.dateRange.to}`);
      printStep(
        '✔',
        `Signals: ${processed.totalSignals}  |  Active days: ${s.activeDays}/${s.totalDays}`
      );
...
function printFinalSummary(results) {
  ...
  const colW = [42, 8, 10];
  const header = ['Screener'.padEnd(colW[0]), 'Stocks'.padEnd(colW[1]), 'Signals'.padEnd(colW[2])];
  console.log(`\n  ${header.join('  ')}`);
  ...
  for (const r of results) {
    ...
    if (hasError) {
      r.errors.forEach((e) => console.log(`    ${e.step}: ${e.error.slice(0, 80)}`));
    }
  }

  const totalStocks = results.reduce((s, r) => s + (r.screener?.rows ?? 0), 0);
  const totalSignals = results.reduce((s, r) => s + (r.backtest?.totalSignals ?? 0), 0);
  ...
  console.log(`  Output folders  : processed/screeners/  &  processed/backtests/`);
  console.log('');
}
